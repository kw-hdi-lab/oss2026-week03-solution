...
async function main() {
  let total = 0;
  try {
    // 콜백 버전은 파일이 3개라 3중첩이었는데, 이렇게 쓰면 파일이 30개여도 루프 하나.
    // forEach 로 돌리면 안 됨 — forEach 는 async 콜백을 기다려 주지 않아서 순서가 섞이고 total 도 틀림.
    ...
    // 세 파일 중 어느 것이 없든 여기 하나로 옴. 콜백 버전은 if (err) 를 세 번 썼음.
    console.error("Failed:", err.message);
    process.exit(1);   // 종료 코드 1. 이게 없으면 실패해도 셸에서는 성공(0)으로 보임.
  }
}
