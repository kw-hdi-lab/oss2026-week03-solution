...
const cachePath = `cache/${name.toLowerCase()}.json`;   // P6. "Busan" → cache/busan.json
...
function paintMax(max) {
  const s = max.toFixed(1);                  // 27 → "27.0". 숫자 그대로 찍으면 소수점이 있다 없다 함.
...
try {
  ...
  const fc = parseForecast(raw);           // 온라인이든 오프라인이든 여기부터는 같음 — P3 에서 둘을 나눈 이유
  ...
  if (flags.includes("--save")) {
    // P6. place 도 같이 저장해야 --offline 때 geocode 를 안 부를 수 있음.
